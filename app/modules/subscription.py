...
def get_or_create_user(email: str) -> int:
    ...
    if db_type == "postgresql":
        ...
        if row:
            ...
            # Create new user
            cursor.execute(
                "INSERT INTO users (email, created_at, last_login_at) VALUES (%s, %s, %s) RETURNING id",
                (email, now, now)
            )
...
def can_perform_search(user_id: int) -> Tuple[bool, Optional[str], Optional[dict]]:
    ...
    Developers (email in DEVELOPER_EMAILS) have unlimited access.
    """
...
def consume_search(user_id: int, subscription_id: Optional[int], search_info: dict) -> None:
    ...
    Args:
        user_id: User ID
        subscription_id: Subscription ID (None for developers)
    ...
    if is_dev or subscription_id is None:
